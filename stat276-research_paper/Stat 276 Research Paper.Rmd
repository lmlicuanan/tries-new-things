...
city_params_sampled.sf <- city_params.sf %>% st_intersection(mc_point_params.sf)

city_params.rs <- city_params.sf %>% st_rasterize()
city_params_sampled.rs <- city_params_sampled.sf %>% st_rasterize()

variogram <- variogram(
  bank ~ restaurant + bar + terminal + hc_facility + pharmacy + school + place_of_worship, 
  city_params_sampled.sf %>% filter(!is.na(bank))
)
...
k = krige(
  bank ~ restaurant + bar + terminal + hc_facility + pharmacy + school + place_of_worship, 
  city_params_sampled.sf %>% st_set_crs("EPSG:4326"),
