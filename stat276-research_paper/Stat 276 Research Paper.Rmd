...
amenity.sf      <- amenity.ql %>% osmdata_sf() %>% trim_osmdata(study_region.bb)
...
mc_point_params.sf <- aggregate(
  ...
  join = function(x, y) st_is_within_distance(x, y, dist = 300)
...
k = krige(
  ...
  city.rs %>% st_set_crs("EPSG:4326"), variogram.model
)

ggplot() + geom_stars(data = k, aes(fill = var1.pred, x = x, y = y)) +
  geom_sf(data = st_cast(city.sf$osm_multipolygons, "MULTILINESTRING"))
```
